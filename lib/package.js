...
var check = require("./exceptions").create(module);

var packageIdCounter = 0;

module.exports = function (serviceMgr, packageInfo, packageJson) {
    check.notNull(serviceMgr, "service manager must be set");
    check.notNull(packageInfo, "package info must be set");
    check.notNull(packageJson, "package json must be set");
